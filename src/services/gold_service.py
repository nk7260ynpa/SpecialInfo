...
提供黃金價格的歷史與最新資料查詢功能。
資料來源為 MySQL SPECIAL_INFO.GoldPrice 資料表。
"""
...
def get_gold_prices(days: int = 30) -> dict:
    """查詢最近 N 天的黃金價格。
    ...
    gold = [_serialize_row(dict(row)) for row in rows]
    return {"gold": gold}


def get_latest_gold_price() -> dict:
    """查詢最新一筆黃金價格。

    Returns:
        包含 gold 最新一筆資料的字典。
    """
    logger.info("查詢最新黃金價格")

    sql = text("""
        SELECT Date, Product, Open, High, Low, Close, Volume
        FROM GoldPrice
        WHERE Date = (SELECT MAX(Date) FROM GoldPrice)
    """)
    ...
    result = {"gold": None}
    for row in rows:
        result["gold"] = _serialize_row(dict(row))
