...
function OilPriceCard({ title, data, latest, loading, color }) {
  // 計算漲跌幅
  ...
  const displayPrice = latest ? latest.close : (data && data.length > 0 ? data[data.length - 1]?.close : null)
  const displayDate = latest ? latest.date : (data && data.length > 0 ? data[data.length - 1]?.date : null)
      ...
      {latest && (
        <div style={styles.info}>
          <div style={styles.infoItem}>
            ...
            <span style={styles.infoValue}>${latest.open.toFixed(2)}</span>
          </div>
          <div style={styles.infoItem}>
            <span style={styles.infoLabel}>最高</span>
            <span style={styles.infoValue}>${latest.high.toFixed(2)}</span>
          </div>
          <div style={styles.infoItem}>
            <span style={styles.infoLabel}>最低</span>
            <span style={styles.infoValue}>${latest.low.toFixed(2)}</span>
          </div>
          <div style={styles.infoItem}>
          ...
          <ResponsiveContainer width="100%" height={200}>
            <LineChart data={chartData}>
              ...
              <Tooltip
                ...
                formatter={(value) => [`$${value.toFixed(2)}`, '收盤價']}
